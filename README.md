...
可视化画布按语言提供不同工具：SVG 偏通用矢量，TikZ 偏数学几何，PGFPlots 偏坐标与函数，Graphviz 偏图节点/边，Mermaid 偏流程关系；原生源码编辑始终保留。内置帮助面板提供科研图、流程图、数据图和几何图方向模板，右侧属性面板可精确修改宽高、旋转、线宽、颜色和层级。对象可直接拖动，四角自由调整宽高，边中点单独调整宽度或高度，`Shift` 等比缩放，上方圆柄连续旋转。箭头头部和描边使用非缩放矢量语义，放大对象不会把线条同步放粗；编辑网格、选择框与控制柄只属于界面层，不会写入最终 SVG 或 Office 插入内容。

### 自定义符号合成

![LaTeXSnipper Office 自定义符号合成器](docs/images/custom-symbol-composer.png)

自定义符号可以组合内置数学符号、任意 LaTeX 图层、矢量图元和自由路径，并独立设置每层位置、宽高、旋转、颜色、透明度与层级。目录支持搜索、分类滚动与翻页；点击画布空白或按 `Esc` 会取消当前选择，但不会删除图层。保存、复制和跨设备传输前仍由 Core 校验冻结合同与 SVG 安全策略。验证通过后可直接插入 Word、Excel 或 PowerPoint，也可生成带 SHA-256 证据的透明 PNG：Obsidian 保存到 Vault 的 `LaTeXSnipper Assets`，VS Code 保存到当前文档旁的 `.latexsnipper-assets`，随后写入 Markdown 图片引用。

### 图片与平台传递

绘图和自定义符号的“多格式复制”会同时写入源码、Core 协议、安全 SVG 和真实 PNG。Windows 使用注册 `PNG` 格式并同时提供 `CF_DIBV5`/`CF_DIB` 回退；macOS/Linux 使用系统图片剪贴板。Obsidian 和 VS Code 支持“发送平台”后自动保存附件并等待插件确认；Typora、Notion、浏览器编辑器及其他未提供附件 API 的平台使用 PNG 剪贴板粘贴，不会被错误标成自动附件插入。
