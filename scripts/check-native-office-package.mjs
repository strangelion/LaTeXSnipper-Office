...
try {
  const signingPath = join(
    stagingDir,
    "certificates",
    "native-office-signing.json",
  );
  ...
  if (!meta.sha1Thumbprint || meta.sha1Thumbprint.length !== 40) {
    console.error(
      "  INVALID: native-office-signing.json missing or invalid sha1Thumbprint",
    );
    process.exit(1);
  }
  ...
  console.error(
    `  INVALID: native-office-signing.json parse error: ${e.message}`,
  );
  process.exit(1);
}
