...
test('keeps explicit point dimensions when they are within constraints', () => {
  const size = new FormulaSvgRenderer()._computeSvgSize(svg({
    width: '120pt',
    height: '30pt',
  ...
  assertAspectRatio(size, 4, 'explicit dimensions must keep their aspect ratio');
});

test('uses the viewBox when MathJax dimensions are absent', () => {
  const size = new FormulaSvgRenderer()._computeSvgSize(svg({ viewBox: '0 0 900 300' }));
...
test('derives a missing dimension from the viewBox', () => {
  ...
  const fromHeight = renderer._computeSvgSize(svg({ height: '24pt', viewBox: '0 0 400 100' }));
  const fromWidth = renderer._computeSvgSize(svg({ width: '60pt', viewBox: '0 0 100 400' }));

  assert.equal(fromHeight.widthPt, 96);
  assert.equal(fromHeight.heightPt, 24);
  assertAspectRatio(fromHeight, 4, 'height-derived width must keep the viewBox ratio');
  assert.equal(fromWidth.widthPt, 60);
  assert.equal(fromWidth.heightPt, 240);
  assertAspectRatio(fromWidth, 0.25, 'width-derived height must keep the viewBox ratio');
});

test('uniformly constrains very wide and very tall formulas', () => {
  const renderer = new FormulaSvgRenderer();
  const wide = renderer._computeSvgSize(svg({ width: '2000pt', height: '100pt' }));
  const tall = renderer._computeSvgSize(svg({ width: '100pt', height: '1000pt' }));
...
test('uniformly raises formulas below the minimum size', () => {
  const size = new FormulaSvgRenderer()._computeSvgSize(svg({ width: '3pt', height: '2pt' }));

  assert.equal(size.widthPt, 18);
  assert.equal(size.heightPt, 12);
  assertAspectRatio(size, 1.5, 'minimum-size scaling must preserve the aspect ratio');
});
