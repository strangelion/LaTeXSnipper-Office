...
export class WordOoxmlHelper {
  buildFormulaOoxml(
    ...
    if (payload.displayMode === "numbered") {
      return this.wrapInFlatOpc(
        this.buildNumberedBody(payload, math, profile),
        payload,
      );
    }
    if (payload.displayMode === "inline") {
      return this.wrapInSdt(math, payload, "run");
    }
    const spacingBefore = Math.max(
      0,
      Math.round((profile.equationSpacingBeforePt ?? 0) * 20),
    );
    const spacingAfter = Math.max(
      0,
      Math.round((profile.equationSpacingAfterPt ?? 0) * 20),
    );
    ...
    return this.wrapInFlatOpc(
      this.wrapInSdt(paragraph, payload, "block"),
      payload,
    );
  }
  ...
  buildReplacementContent(
    ...
    const math = this.normalizeOmml(omml, payload.displayMode !== "inline");
    if (payload.displayMode === "numbered")
      return this.buildNumberedBody(payload, math, profile);
    if (payload.displayMode === "inline")
      return this.wrapInSdt(math, payload, "run");
    return this.wrapInSdt(
      `<w:p><w:pPr><w:jc w:val="center"/><w:ind w:firstLine="0"/><w:keepLines/></w:pPr>${math}</w:p>`,
      payload,
      "block",
    );
  }
  ...
  extractFormulaId(xml: string): string | null {
    const match = xml.match(
      /<w:tag\s+w:val="latexsnipper:formula:([^"<>]+)"\s*\/>/,
    );
    return match?.[1] ?? null;
  }
  ...
  extractText(xml: string): string {
    ...
    while ((match = regex.exec(xml)) !== null)
      texts.push(this.decodeXml(match[1]));
    return texts.join("");
  }
  ...
  private buildNumberedBody(
    ...
    const sideWidthPct = Math.max(
      500,
      Math.min(
        1250,
        Math.round(((profile.numberColumnMinWidthPt ?? 54) / 468) * 5000),
      ),
    );
    const centerWidthPct = 5000 - sideWidthPct * 2;
    const bookmark = bookmarkNameForFormula(payload.formulaId);
    const bookmarkId = bookmarkNumericIdForFormula(payload.formulaId);
    const visible =
      profile.numberingScheme === "global"
        ? "1"
    ...
    const cell = (
      ...
      `<w:tc><w:tcPr><w:tcW w:w="${width}" w:type="pct"/><w:vAlign w:val="center"/><w:tcBorders>${nilBorders}</w:tcBorders></w:tcPr>` +
  ...
  private buildNumberFields(
    ...
    const heading = this.escapeXml(
      profile.chapterStyle ?? `Heading ${profile.chapterLevel ?? 1}`,
    );
  ...
  private normalizeOmml(omml: string, display: boolean): string {
    ...
    if (display) {
      ...
      if (cleaned.startsWith("<m:oMath"))
        return `<m:oMathPara>${cleaned}</m:oMathPara>`;
  ...
  private wrapInSdt(
    ...
    return `<w:sdt xmlns:w="${W_NS}" xmlns:m="${M_NS}"><w:sdtPr><w:alias w:val="LaTeXSnipper Formula"/><w:tag w:val="${formulaTag(payload.formulaId)}"/><w:showingPlcHdr w:val="0"/></w:sdtPr><w:sdtContent>${body}</w:sdtContent></w:sdt>`;
  }

  buildMetadataXml(payload: OfficeFormulaPayload): string {
    const metadata = encodeFormulaMetadata(payload);
    return `<lsn:formula xmlns:lsn="${LSN_NS}" formulaId="${this.escapeXml(payload.formulaId)}"><lsn:payload>${metadata}</lsn:payload></lsn:formula>`;
  }
