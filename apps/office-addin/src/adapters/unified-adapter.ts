...
export class OfficeHostAdapter implements HostAdapter {
  ...
  async execute(command: Command): Promise<CommandResult> {
    ...
    if (!adapter)
      return {
        ...
        error: "This Microsoft Office host is not supported.",
      };

    switch (command.type) {
      case "InsertFormula":
        return this.toCommandResult(
          await adapter.insertFormula(this.payloadFromCommand(command.payload)),
        );
      ...
      case "ReplaceSelectedFormula": {
        ...
        if (!selected.ok || !selected.data)
          return this.toCommandResult(selected);
        return this.toCommandResult(
          await adapter.replaceSelectedFormula(
            this.payloadFromCommand({
              ...command.payload,
      ...
      case "InsertEquationReference":
        return this.toCommandResult(
          await adapter.insertEquationReference(command.payload.formulaId),
        );
      ...
      case "ReplaceSelection":
        return {
          ok: false,
          code: "UNSAFE_GENERIC_REPLACE",
          error:
            "Generic selection replacement is disabled for formula operations. Use ReplaceSelectedFormula.",
        };
      case "ConvertToLaTeX": {
        try {
          return {
            ok: true,
            data: await this.bridge.convert(
              "omml",
              "latex",
              command.payload.omml,
              "inline",
            ),
          };
        } catch (error) {
      ...
      case "RenderPreview": {
        try {
          const format =
            command.type === "RenderFormula"
              ? (command.payload.format ?? "png")
              : "svg";
          const displayMode: FormulaDisplayMode =
            command.type === "RenderFormula"
              ? (command.payload.display ?? "block")
              : "block";
          return {
            ok: true,
            data: await this.bridge.convert(
      ...
      case "DetectTable":
        return {
          ...
          error: "Table extraction is not supported by this Office.js adapter.",
        };
      case "FormatContent":
        return {
          ok: false,
          ...
          error:
            "Global formatting is intentionally not applied by the Office.js formula adapter.",
        };
      case "OpenEditor":
      case "OpenSettings":
        return { ok: true };
  ...
  private currentAdapter(): OfficeFormulaHostAdapter | null {
    ...
    return host === "word" || host === "excel" || host === "powerpoint"
      ? this.adapters[host]
      : null;
  }
  ...
  private toCommandResult(result: {
    ok: boolean;
    data?: unknown;
    error?: string;
    code?: string;
    ...
    return result.ok
      ...
      : {
          ok: false,
          error: result.error ?? "Office operation failed",
          code: result.code,
        };
  }
