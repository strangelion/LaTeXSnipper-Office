...
export class WordOfficeAdapter implements OfficeDocumentAdapter {
  ...
  async getHostCapabilities(): Promise<HostCapabilities> {
    ...
    return {
      ...
      platform,
      version:
        Office && Office.context && Office.context.diagnostics
   ...
   */
  private async extractFormulaFromOoxml(
    xml: string,
  ): Promise<EquationBlock | null> {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.latex) {
          return {
            math: { type: "latex" as const, content: data.latex },
            display,
            numbered: false,
          };
        }
      ...
      console.warn(
        "[OfficeAddin] operation=convert-omml host=word formulaId=<unknown> hresult=0x00000000",
        error,
      );
    }
    ...
    return {
      math: { type: "omml" as const, content: omml },
      display,
      numbered: false,
    };
  }
   ...
   */
  private async extractTableData(_table: any, _context: any): Promise<TableIR> {
   ...
   */
  private async buildFormulaOoxml(
   ...
   */
  private async buildNumberedEquationOoxml(
    request: InsertFormulaRequest,
  ): Promise<string> {
    const mathContent = await this.extractOmml(request.fragment);
    const body = this.wrapInSdt(
  ...
  private async buildEquationOoxml(formula: EquationBlock): Promise<string> {
    ...
    const body = this.wrapInSdt(
      `<w:p><${mathTag}>${content}</${mathTag}></w:p>`,
    );
    return this.wrapInFlatOpc(body);
  }
    ...
    for (const block of fragment.blocks) {
      if (block.type === "equation") {
        ...
        if (block.math.type === "latex") {
          ...
          const omml = await this.latexToOmml(
            block.math.content,
            block.display,
          );
          return omml;
  ...
  private async detectPlatform(): Promise<"windows" | "mac" | "web"> {
    try {
      ...
      console.warn(
        "[OfficeAddin] operation=detect-platform host=word formulaId=<unknown> hresult=0x00000000",
        error,
      );
    }
    return "web";
