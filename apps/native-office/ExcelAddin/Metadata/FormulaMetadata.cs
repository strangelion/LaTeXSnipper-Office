...
/// <summary>
/// Manages formula metadata in Excel Workbook CustomXMLParts.
...
public static class FormulaMetadata
{
    ...
    /// <summary>
    /// Write formula metadata to workbook CustomXMLParts.
    /// </summary>
    public static void Write(Workbook workbook, FormulaPayload payload)
    {
        try
        {
...
<lsno:noffice xmlns:lsno=""{NamespaceUri}"">
  <lsno:formula id=""{payload.FormulaId}"" version=""2"" created=""{DateTime.UtcNow:O}"">
    ...
    /// </summary>
    public static FormulaPayload? Read(Workbook workbook, string formulaId)
    {
        try
        {
    ...
    /// </summary>
    public static void Update(Workbook workbook, string formulaId, FormulaPayload payload)
    {
        Remove(workbook, formulaId);
        Write(workbook, payload);
    }

    /// <summary>
    /// Remove metadata for a formula.
    /// </summary>
    public static void Remove(Workbook workbook, string formulaId)
    {
        try
        {
            for (int i = workbook.CustomXMLParts.Count; i >= 1; i--)
            {
    ...
    /// </summary>
    public static int CleanupOrphans(Workbook workbook, HashSet<string> activeFormulaIds)
    {
        int removed = 0;
        try
        {
