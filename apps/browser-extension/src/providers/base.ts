...
function blocks(element: Element): ExtractedContentBlock[] {
  ...
  for (const child of Array.from(
    element.querySelectorAll("p,h1,h2,h3,h4,h5,h6,li,blockquote,pre,table"),
  )) {
    if (!visible(child)) continue;
    const tag = child.tagName.toLowerCase();
    const type: ExtractedContentBlock["type"] =
      tag === "pre"
        ? "code"
        : tag === "table"
          ? "table"
  ...
  return result.length
    ...
    : [
        {
          type: "paragraph",
          text: (element.textContent || "").slice(0, 64 * 1024),
        },
      ];
}
...
export function createProviderAdapter(
  ...
  const listMessages = (root: Element): Element[] => {
    for (const selector of definition.messages) {
      try {
        ...
        if (found.length) return found.slice(0, 500);
      } catch {
        /* audited selector */
      }
    }
  ...
  const identifyRole = (element: Element): MessageRole => {
    ...
    const label =
      `${element.getAttribute("data-message-author-role") || ""} ${element.getAttribute("aria-label") || ""}`.toLowerCase();
  ...
  return {
    ...
    detectCapabilities(document: Document): ProviderCapabilities {
      ...
      return {
        ...
        readSelection: true,
        readCurrentMessage: count > 0,
        readVisibleConversation: count > 0,
        readLoadedConversation: count > 0,
        extractFormulas: count > 0,
        extractCode: count > 0,
        extractTables: count > 0,
        insertIntoComposer: !!this.findComposer(document),
        streamObservation: !!root,
      };
    },
    ...
    extractMessage(
      ...
      const codeBlocks = Array.from(element.querySelectorAll("pre code, pre"))
        .filter(visible)
        .slice(0, 50)
        .map((node) => ({
          language: node.getAttribute("data-language") || undefined,
          code: (node.textContent || "").slice(0, 128 * 1024),
        }));
      ...
      return {
        id,
        providerMessageId: element.getAttribute("data-message-id") || undefined,
        role,
        ...
        formulas: extractFormulaCandidates(element, {
    ...
    diagnostics(document: Document) {
      ...
      return root
        ? [
            {
              code: "PROVIDER_MATCHED",
              strategy: definition.roots.find((selector) => {
                try {
              ...
              message: "Visible conversation root matched.",
            },
          ]
        : [
            {
              ...
              message:
                "No audited conversation root matched; use selection-only mode.",
            },
          ];
    },
    observeStreaming(
      ...
      const observer = new MutationObserver((mutations) => {
        for (const mutation of mutations) {
          ...
          if (target && !target.closest("[data-latexsnipper-overlay]"))
            pending.add(target);
        ...
        timer = window.setTimeout(() => {
          callback(Array.from(pending).slice(0, 5000));
          pending.clear();
        }, 350);
      });
