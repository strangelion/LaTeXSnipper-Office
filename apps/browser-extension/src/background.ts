...
async function bridge(): Promise<BridgeClient> {
  if (!bridgePromise)
    bridgePromise = (async () => {
      ...
      const clientId =
        typeof stored.browserClientId === "string"
          ? stored.browserClientId
          : `browser-${crypto.randomUUID()}`;
      if (!stored.browserClientId)
        await chrome.storage.local.set({ browserClientId: clientId });
...
async function enqueueImport(action: BrowserImportAction): Promise<unknown> {
  if (action.origin !== "browser" || action.target !== "desktop")
    throw new Error("INVALID_ACTION_DIRECTION");
  if (
    !["ImportWebFormula", "ImportConversationSelection"].includes(
      action.actionType,
    )
  )
    throw new Error("INVALID_IMPORT_ACTION");
...
async function pollDesktopActions(preferredTabId?: number): Promise<void> {
  ...
  try {
    ...
    if (
      !["InsertFormulaIntoBrowser", "ReplaceBrowserSelection"].includes(
        action.actionType,
      )
    ) {
      await client.complete(action.actionId, false, undefined, {
        code: "UNSUPPORTED_BROWSER_ACTION",
        message:
          "Only versioned desktop-to-browser insertion actions are accepted.",
      });
      return;
    }
    ...
    await client.complete(
      ...
      result?.ok
        ? undefined
        : {
            code: result?.errorCode || "BROWSER_INSERT_FAILED",
            message: result?.message || "Browser editor rejected insertion.",
          },
    );
  } catch (error) {
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  if (!tab?.id) return;
  if (info.menuItemId === "insert-formula") {
    activeUiCount = Math.max(1, activeUiCount);
    await pollDesktopActions(tab.id);
    return;
  }
...
chrome.runtime.onMessage.addListener(
  (message: unknown, _sender, sendResponse) => {
    ...
    (async () => {
      if (request.type === "UI_ACTIVE") {
        activeUiCount += 1;
        const client = await bridge();
        await client.register(VERSION);
        sendResponse({ ok: true });
        return;
      }
      if (request.type === "UI_INACTIVE") {
        activeUiCount = Math.max(0, activeUiCount - 1);
        sendResponse({ ok: true });
        return;
      }
      if (request.type === "BRIDGE_PING") {
        try {
          await (await bridge()).ping();
          sendResponse({ ok: true });
        } catch (error) {
          sendResponse({
            ok: false,
      ...
      if (request.type === "SEND_IMPORT" && request.action) {
        await enqueueImport(request.action);
        sendResponse({ ok: true });
        return;
      }
