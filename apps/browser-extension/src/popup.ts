...
async function background(message: unknown): Promise<BackgroundResponse> {
...
async function scan(message: unknown): Promise<void> {
  ...
  const formulaCount =
    "formulas" in payload
      ? payload.formulas.length
      : payload.messages.reduce(
          (total, item) =>
            total +
            item.blocks.filter((block) => block.type === "formula").length,
          0,
        );
  const messageCount = "messages" in payload ? payload.messages.length : 0;
  const preview = document.createElement("div");
  preview.className = "result";
  preview.textContent = `${t("messagesCount", String(messageCount))} · ${t("formulasCount", String(formulaCount))}`;
  const confirm = document.createElement("button");
  confirm.textContent = t("sendSelection");
  confirm.addEventListener("click", async () => {
    const sent = await background({
      type: "SEND_IMPORT",
      action: response.action,
    });
    if (!sent.ok) throw new Error(sent.error);
    setStatus(t("sentToDesktop"), true);
...
document
  .getElementById("scanSelection")!
  .addEventListener(
    "click",
    () =>
      void scan({ type: "SCAN_SELECTION" }).catch(() =>
        setStatus(t("noSelection")),
      ),
  );
document.getElementById("scanMessage")!.addEventListener(
  "click",
  () =>
    void scan({
      type: "SCAN_CONVERSATION",
      mode: "current-assistant-message",
    }).catch(() => setStatus(t("scanFailed"))),
);
document
  ...
  .addEventListener(
    "click",
    () =>
      void scan({ type: "SCAN_PAGE_FORMULAS" }).catch(() =>
...
(async () => {
  ...
  const bridge = await background({ type: "BRIDGE_PING" });
  setStatus(bridge.ok ? t("connected") : t("desktopOffline"), !!bridge.ok);
})().catch(() => setStatus(t("desktopOffline")));
window.addEventListener("unload", () => {
  void background({ type: "UI_INACTIVE" });
});
