...
export interface ReadScopeSnapshot {
  mode: ReadScopeMode;
  roles: MessageRole[];
  messageIds: string[];
...
export type InlineRun = {
  type: "text" | "bold" | "italic" | "code" | "link" | "formula";
  text?: string;
  href?: string;
...
export interface ConversationImportMessage {
  ...
  sequence: number;
  language?: string;
  blocks: ConversationImportBlock[];
}
export interface ConversationImportDocument {
  ...
  extractedAt: string;
  scope: ReadScopeSnapshot;
  messages: ConversationImportMessage[];
  truncated: boolean;
  truncation?: TruncationInfo;
  diagnostics: ExtractionDiagnostic[];
}
export interface FormulaImportPayload {
  schemaVersion: 1;
...
export interface BrowserImportAction {
  schemaVersion: 1;
  actionType: "ImportWebFormula" | "ImportConversationSelection";
  origin: "browser";
  target: "desktop";
  source: {
