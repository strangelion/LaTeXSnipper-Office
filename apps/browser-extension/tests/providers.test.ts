...
describe("provider adapters", () => {
  it("has independent dedicated adapters and honest unverified maturity", () => {
    ...
    expect(PROVIDERS.every((provider) => provider.verifiedLive === false)).toBe(
      true,
    );
  });
  for (const provider of PROVIDERS) {
    it(`${provider.id} matches its production host and extracts bounded visible fixtures`, () => {
      ...
      expect(providerFor(new URL(`https://${host}/fixture`)).id).toBe(
        provider.id,
      );
      ...
      if (messages.length)
        expect(
          provider.extractMessage(messages[0], {
            pageUrl: `https://${host}/fixture`,
            sequence: 0,
          })?.text.length,
        ).toBeGreaterThan(0);
