...
Word Native 编号目前只使用全局 `SEQ LaTeXSnipperEquation`。布局按实际正文、分栏或表格单元格宽度计算 center/right tab stop，保留用户已有制表位，并将前导 Tab、OLE、编号字段和 Bookmark 纳入同一 owned range。章节点号和章节连字符 profile 目前只由 Office.js Word 提供。
