...
- 全局编号使用命名序列 `SEQ LaTeXSnipperEquation`；章节编号支持 `2.1` 与 `2-1`，通过 `STYLEREF` 与按标题级别重启的 `SEQ` 组成；
...
Excel 在当前 worksheet 插入 PNG shape，PowerPoint 在唯一选中 slide 插入 PNG shape。shape 的名称和 alt-text metadata 保存 `formulaId`、LaTeX 与显示模式，因此支持选择、读取、原位替换和删除。
...
Word Native 编号布局按实际正文、分栏或表格单元格宽度计算 center/right tab stop；字段、书签和局部段落格式不会覆盖用户全局样式。
...
| Office.js Excel/PowerPoint | TypeScript build、禁止纯文本 fallback 检查 | 真实 shape API、选择读取与替换 |
