...
Windows Office.js manifest 通过每宿主 WEF 注册项安装并写入 refresh marker；macOS 分别写入 Word、Excel、PowerPoint sandbox container 的 `Data/Documents/wef/LaTeXSnipper.xml`。安装会先在 `com.latexsnipper.office/localhost-certs` 创建本地证书，通过 macOS login Keychain 建立并复验 `127.0.0.1` 的 SSL 信任，再验证和写入 owned manifest；任何 TLS 信任失败都会中止安装并返回 `OFFICEJS_TLS_TRUST_FAILED`，不会报告伪成功。设置页将状态区分为 `not-installed`、`manifest-installed`、`tls-untrusted`、`ready` 与 `connected`；Office 应用探测仅用于展示，不作为安装或环境就绪的硬门槛。
